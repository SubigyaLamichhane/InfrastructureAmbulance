...
export class ComplainResolver {
  ...
  async complainsByCategory(
    ...
    complain.where('p."category" = :category', {
      category,
    });
    const complains = await complain.getMany();
  ...
  @Query(() => PaginatedComplains)
  async complainsByWard(
    @Arg('limit', () => Int) limit: number,
    @Arg('cursor', () => String, { nullable: true }) cursor: string | null,
    @Arg('wardNo', () => Int) wardNo: number
  ): Promise<PaginatedComplains> {
    const realLimit = limit >= 50 ? 50 : limit;
    const realLimitPlusOne = realLimit + 1;
  ...
  @Mutation(() => Boolean)
  async deleteComplain(@Arg('id', () => Int) id: number): Promise<boolean> {
    try {
  ...
  @Mutation(() => Boolean)
  async markCompleted(@Arg('id', () => Int) id: number): Promise<boolean> {
    let completedComplain: CompletedComplain;
    try {
      // clone complain into CompletedComplain
      const complain = await Complain.findOne({ where: { id } });
      if (!complain) {
        return false;
      }
      completedComplain = await CompletedComplain.create({
        ...complain,
      }).save();
    ...
    return completedComplain ? true : false;
  }
}
